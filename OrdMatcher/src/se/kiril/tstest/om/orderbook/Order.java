...
//	private final Time expireTime;
	...
	public ExecutionReport triggerEndOfDay() {
		...
		ExecutionReport report = new ExecutionReport(ExecType.DONE_FOR_DAY, this);
		return report;
	}
	public ExecutionReport queryOrdStatus() {
		ExecutionReport report = new ExecutionReport(ExecType.ORDER_STATUS, this);
		return report;
	}
	/**
	 * Trades the initiator order pOrd with the target order producing two execution reports.
	 * @param pOrd Initiator order
	 * @param pLimitPrice trade price
	 * @param pVol trade volume
	 * @return Returns an object array where [0]=Modified initiator order, [1]=Initiator order execution report, [2]=Target order execution report
	 */
	public Order trade(Order pOrd, Double pLimitPrice, int pVol) {
		Order initOrder = pOrd;
		if(ordStatus.equals(OrdStatus.NEW) || ordStatus.equals(OrdStatus.PARTIALLY_FILLED)){
			if (pVol <= leavesQty){
				leavesQty -= pVol;
				cumQty += pVol;
				lastQty = pVol;
				...
				ExecutionReport ordReport = new ExecutionReport(ExecType.TRADE, this);		
				ExecutionReport initiatorOrdReport = new ExecutionReport(ExecType.TRADE, initOrder);
				...
				initOrder.lastPx = pLimitPrice;
				initOrder.grossTradeAmt += initOrder.lastQty * initOrder.leavesQty;
				if (initOrder.leavesQty <= 0){
					initOrder.ordStatus = OrdStatus.FILLED;
				}else {
					initOrder.ordStatus = OrdStatus.PARTIALLY_FILLED;
			...
			//System.err.println("Cannot trade with "+this.ordId+", the order is "+ ordStatus.toString());
			return null;
		}		
	}
	...
	public Double getLimitPx() {return limitPx;}
	public int getQty() {return ordQty;}
