...
public enum OrdStatus {
	 ...
	 * but has not yet occurred. For example, a specialist on an exchange may "stop"
	 * an order while searching for a better price. 
	 */
	STOPPED,
	/**
	 * Order has been rejected by sell-side (broker, exchange,
	 * ECN). NOTE: An order can be rejected subsequent to
	 * order acknowledgment, i.e. an order can pass from New
	 * to Rejected status.
	 */
	REJECTED,
	/**
	 * The order is not eligible for trading. This usually happens as a
	...
	SUSPENDED,
	/**
	 * Order has been received by sell-side's (broker,
	 * exchange, ECN) system but not yet accepted for
	 * execution. An execution message with this status will
	 * only be sent in response to a Status Request message.
	 */
	PENDING_NEW,
	/**
	 ...
	 * pricing. It is anticipated that this status will only be
	 * used with the "Disclosed" BidType List Order Trading
	 * model.
	 */
